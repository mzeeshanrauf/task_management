...
def get_columns(filters=None):
    ...
    if filters.get("show_task_kpi"):
        cols += [
            {"fieldname": "total_tasks",      "label": _("Total Tasks"),        "fieldtype": "Int",                                 "width": 90},
            {"fieldname": "completed_tasks",  "label": _("Completed"),          "fieldtype": "Int",                                 "width": 90},
            {"fieldname": "on_time_tasks",    "label": _("On Time"),            "fieldtype": "Int",                                 "width": 80},
            ...
            {"fieldname": "quality_score_100","label": _("Quality (0-100)"),    "fieldtype": "Float",                               "width": 110},
            {"fieldname": "task_kpi_score",   "label": _("Task KPI Score"),     "fieldtype": "Float",                               "width": 110},
            {"fieldname": "task_kpi_grade",   "label": _("Task Grade"),         "fieldtype": "Data",                                "width": 90},
            {"fieldname": "task_kpi_rating",  "label": _("Task Rating"),        "fieldtype": "Data",                                "width": 160},
        ]
    return cols
...
def get_data(filters, from_date, to_date):
    ...
    sales_persons = frappe.db.sql(f"""
        SELECT sp.name AS sales_person,
               sp.custom_user AS user_id,
               u.full_name AS employee_name,
               sp.employee,
               e.department
        FROM `tabSales Person` sp
        LEFT JOIN `tabUser` u ON u.name = sp.custom_user
        LEFT JOIN `tabEmployee` e ON e.user_id = sp.custom_user
        WHERE sp.enabled = 1
          AND (
    ...
    for sp in sales_persons:
        ...
        # Task KPI — only when checkbox is checked
        if filters.get("show_task_kpi") and (sp.employee or sp.user_id):
            from task_management.task_management.doctype.employee_task.employee_task import (
                get_employee_task_summary
            )
            s = get_employee_task_summary(sp.employee or "", from_date, to_date)
            has_tasks = (s.get("total_tasks") or 0) > 0
            task_kpi  = s.get("task_kpi_score") or 0
            row.update({
                "total_tasks":       s.get("total_tasks", 0),
                "completed_tasks":   s.get("completed", 0),
                "on_time_tasks":     s.get("on_time", 0),
                "completion_rate":   s.get("completion_rate", 0),
                "on_time_rate":      s.get("on_time_rate", 0),
                "quality_score_100": s.get("quality_score_100", 0),
